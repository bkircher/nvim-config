...
local function deno_fmt()
  ...
  local output, code
  if vim.system then
    local res = vim.system({ 'deno', 'fmt', ('--ext=%s'):format(ext), '-' }, { stdin = input }):wait()
    output = res.stdout and res.stdout or ''
    code = res.code or 1
  else
    output = table.concat(vim.fn.systemlist({ 'deno', 'fmt', ('--ext=' .. ext), '-' }, input), '\n')
...
vim.api.nvim_create_user_command('DenoFmt', deno_fmt, { desc = 'Format current buffer with deno fmt' })
